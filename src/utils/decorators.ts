...
export function step<This, Args extends any[], Return>(message: string, fn?: (args: Args) => unknown) {
  return function actualDecorator(
    ...
    async function replacementMethod(this: any, ...args: Args) {
      ...
      const hasFn = fn && hasArgs ? fn(args) : '';

      const argsText = hasArgs && hasFn ? fn!(args) : hasArgs ? JSON.stringify(args[0]) : '';

      const name = `【${this.constructor.name}】 ${message}`;
      const prettyName = name.includes('_$') ? name.replace('_$', JSON.stringify(args)) : `${name}  ${argsText}`;
